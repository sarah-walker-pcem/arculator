...
static void *scsi_cd_init(scsi_bus_t *bus, int id, podule_t *podule)
{
        const char *fn;
        char config_name[20];
...
//                        pclog("Read sector failed\n");
		...
		{
                        aka31_log("Unimplemented mode sense %02x\n", c);
...
//                case GPCMD_PLAY_AUDIO_12_OLD:
		...
		if ((data->cd_status <= CD_STATUS_DATA_ONLY) ||
...
//                aka31_log("GPCMD_PLAY_AUDIO_TRACK_INDEX: start=%i end=%i size=%i\n", cdb[4], cdb[7], atapi->size());
                {
                        uint8_t buf[12];
...
//                        aka31_log("Read subchannel complete - audio status %02X\n",data->data_in[1]);
                        len = 11+5;
                        if (!(cdb[2] & 0x40)) len=4;
...
//                aka31_log(" cdb[1]=%02x page_code=%02x\n", cdb[1], page_code);
		if (cdb[1] & 1)
		{
			preamble_len = 4;
			size_idx = 3;
...
atapi_out:
		...
		return alloc_length ? SCSI_PHASE_DATA_IN : SCSI_PHASE_STATUS;
                ...
                switch (cdb[1] & 3)
                {
                        ...
                        default:
                        fatal("GPCMD_TOSHIBA_READ_DISC_INFO format %i %02x\n", cdb[1] & 3, cdb[2]);
                }
                data->cmd_pos = CMD_POS_IDLE;
                return SCSI_PHASE_DATA_IN;
                ...
                default:
                aka31_log("Unknown CD command %02x\n", cdb[0]);
		atapi_cmd_error(data, SENSE_ILLEGAL_REQUEST, ASC_ILLEGAL_OPCODE, 0);
                return SCSI_PHASE_STATUS;
        }
}
