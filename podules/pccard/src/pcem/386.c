...
//                while (cycdiff < cycle_period)
                ...
                if (!cpu_state.abrt)
                {
                        ...
                        if (output == 3)
                        {
                                pclog("%04X(%06X):%04X : %08X %08X %08X %08X %04X %04X %04X(%08X) %04X %04X(%08x) %04X(%08X) %08X %08X %08X SP=%04X:%08X %02X %04X %i   %02X %02x %02x %02X\n",CS,cs,cpu_state.pc,EAX,EBX,ECX,EDX,CS,DS,ds,ES,es,FS,GS,SS,ss,EDI,ESI,EBP,SS,ESP,opcode,cpu_state.flags,ins, ram[0x600+0xfc], ram[0x600+0xfd], ram[0x600+0xfe], ram[0x600+0xff]);
                        }
                        cpu_state.pc++;
